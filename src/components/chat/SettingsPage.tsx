...
function FormInput({ label, value, onChange, placeholder, type = "text" }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; type?: string }) {
  return (
    <div>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="w-full rounded-xl border border-[rgba(168,135,86,0.2)]/60 bg-[#F8F8F6] px-3 py-2.5 text-sm text-[#1A1A1A] outline-none transition-colors placeholder:text-[#7A7A7A]/40 focus:border-copper/50 focus:ring-2 focus:ring-copper/10 sm:px-4 sm:py-3" />
    </div>
  );
}
...
function FormTextarea({ label, value, onChange, placeholder, rows = 3 }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; rows?: number }) {
  return (
    <div>
      <label className="mb-1.5 block text-xs font-medium text-[#7A7A7A] sm:text-sm">{label}</label>
      <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows} className="w-full resize-y rounded-xl border border-[rgba(168,135,86,0.2)]/60 bg-[#F8F8F6] px-3 py-2.5 text-sm text-[#1A1A1A] outline-none transition-colors placeholder:text-[#7A7A7A]/40 focus:border-copper/50 focus:ring-2 focus:ring-copper/10 sm:px-4 sm:py-3" />
    </div>
  );
}
...
function SelectInput({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: { value: string; label: string }[] }) {
  return (
    <div>
      <label className="mb-1.5 block text-xs font-medium text-[#7A7A7A] sm:text-sm">{label}</label>
      <select value={value} onChange={e => onChange(e.target.value)} className="w-full appearance-none rounded-xl border border-[rgba(168,135,86,0.2)]/60 bg-[#F8F8F6] px-3 py-2.5 text-sm text-[#1A1A1A] outline-none transition-colors focus:border-copper/50 focus:ring-2 focus:ring-copper/10 sm:px-4 sm:py-3">
...
function SettingRow({ icon: Icon, label, onClick, badge, trailing }: { icon: any; label: string; onClick?: () => void; badge?: string | number; trailing?: React.ReactNode }) {
  return (
    <div onClick={onClick} className={`flex items-center gap-3 rounded-xl p-3 transition-colors sm:gap-4 sm:p-3.5 ${onClick ? "cursor-pointer hover:bg-[#F8F8F6]" : ""}`}>
      ...
      {badge !== undefined && <span className="rounded-full bg-copper/12 px-2 py-0.5 text-[11px] font-medium text-copper">{badge}</span>}
      ...
      <div className="flex h-full flex-col overflow-auto bg-[var(--color-bg)]">
        <div className="w-full px-4 py-6 sm:px-5 sm:py-8 lg:px-6">
          ...
          <Card className="mb-4">
            ...
            <div className="mt-5">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                <input type="number" min={0} max={10000} value={aiCfg.reply_min_chars} onChange={e => setAiCfg(p => ({ ...p, reply_min_chars: Math.max(0, parseInt(e.target.value) || 0) }))} className="w-20 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-center text-sm text-[var(--color-text)] outline-none focus:border-copper/50 sm:w-24" />
                <span className="text-sm text-[var(--color-text-secondary)]">~</span>
                <input type="number" min={0} max={10000} value={aiCfg.reply_max_chars} onChange={e => setAiCfg(p => ({ ...p, reply_max_chars: Math.max(0, parseInt(e.target.value) || 0) }))} className="w-20 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-center text-sm text-[var(--color-text)] outline-none focus:border-copper/50 sm:w-24" />
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleTestAi} disabled={aiTesting || !aiCfg.api_url || !aiCfg.api_key} className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-copper/30 bg-[var(--color-surface)] px-4 py-3 text-sm font-medium text-copper transition-all hover:bg-[var(--color-accent-light)] disabled:opacity-40">{aiTesting ? <><Loader size={16} strokeWidth={2} className="animate-spin" /> {t("ai.testing", lang)}</> : t("ai.test", lang)}</button>
          </div>
                  ...
                  <div key={p.id}>
                    <div className="flex items-center gap-3 rounded-2xl bg-[var(--color-surface)] p-3.5 shadow-sm sm:p-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {p.skills && p.skills.length > 0 && <div className="mt-1.5 flex flex-wrap gap-1">{p.skills.map((sid: string) => { const skill = allSkills.find((s: any) => s.id === sid); return skill ? <span key={sid} className="rounded bg-copper/10 px-1.5 py-0.5 text-[10px] font-medium text-copper">{skill.name}</span> : null; })}</div>}
                      </div>
                      <div className="flex shrink-0 flex-col gap-1.5 sm:flex-row sm:gap-2">
                        <button onClick={() => { setEditingPersona(p); setPage("personaEdit"); }} className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-accent-light)] text-copper transition-colors hover:bg-[var(--color-accent-light)]"><Edit3 size={14} strokeWidth={1.5} /></button>
        ...
        <div className="w-full px-4 py-6 sm:px-5 sm:py-8 lg:px-6">
          ...
          <Card className="mb-4">
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      <input type="number" min={0} max={100} value={affectionValue} onChange={e => setAffectionValue(Math.max(0, Math.min(100, parseInt(e.target.value) || 0)))} className="w-20 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2.5 text-center text-sm text-[var(--color-text)] outline-none transition-colors focus:border-copper/50 focus:ring-2 focus:ring-copper/10" />
            ...
            <div className="mt-5">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">{allSkills.map(s => { const isSelected = selectedSkills.includes(s.id); return (<div key={s.id} onClick={() => setSelectedSkills(prev => prev.includes(s.id) ? prev.filter(id => id !== s.id) : [...prev, s.id])} className={`flex cursor-pointer items-center gap-3 rounded-xl p-3 transition-colors ${isSelected ? "border border-copper/30 bg-[var(--color-accent-light)]" : "border border-transparent bg-[var(--color-bg)]/50"}`}><div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded text-xs font-semibold ${isSelected ? "bg-gradient-to-r from-[#1A1A1A] to-[#3A3A3A] text-white" : "border-2 border-gray-300 text-transparent"}`}>{isSelected ? "✓" : ""}</div><div className="min-w-0 flex-1"><div className="text-[13px] font-medium text-[var(--color-text)]">{s.name}</div><div className="mt-0.5 text-[11px] text-[var(--color-text-secondary)]">{s.description}</div></div><span className="shrink-0 rounded bg-[var(--color-accent-light)] px-1.5 py-0.5 text-[10px] font-medium text-copper">{s.type === "thinking" ? "思维" : s.type === "conversation" ? "话术" : "感知"}</span></div>); })}</div>
      ...
      <div className="flex h-full flex-col overflow-auto bg-[var(--color-bg)]">
        <div className="w-full px-4 py-6 sm:px-5 sm:py-8 lg:px-6">
          <PageHeader title={t("ip.title", lang)} onBack={() => setPage("account")}>
            <button onClick={loadIpData} className="rounded-lg bg-[var(--color-accent-light)] px-2.5 py-1.5 text-xs text-copper">{t("common.refresh", lang)}</button>
          </PageHeader>
          ...
          <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[{ label: t("ip.total", lang), value: ipStats.total, color: "text-[var(--color-text)]" },
              ...
              { label: t("ip.recent", lang), value: ipStats.recent, color: "text-copper" }
            ].map(s => (
          ...
          <div className="mb-4 flex gap-2">
            <input value={ipSearch} onChange={e => setIpSearch(e.target.value)} placeholder={t("ip.search_ip", lang)} className="flex-1 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm text-[var(--color-text)] outline-none focus:border-copper/50" />
          ...
          <Card className="mb-4">
            ...
            {settings.notify_quiet_enabled && (
              <div className="mt-3 flex items-center gap-2 pl-[58px]">
                <input type="time" value={settings.notify_quiet_start} onChange={e => handleSaveSettings({ notify_quiet_start: e.target.value })} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-2 py-1.5 text-sm text-[var(--color-text)] outline-none focus:border-copper/50" />
                <span className="text-sm text-[var(--color-text-secondary)]">{t("notify.quiet_to", lang)}</span>
                <input type="time" value={settings.notify_quiet_end} onChange={e => handleSaveSettings({ notify_quiet_end: e.target.value })} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-2 py-1.5 text-sm text-[var(--color-text)] outline-none focus:border-copper/50" />
              </div>
            )}
          ...
          <Card className="mb-4">
            ...
            <div className="mt-3 grid grid-cols-4 gap-2">
              {[{ v: "0", l: t("privacy.never", lang) }, { v: "7", l: `7${t("common.days", lang)}` }, { v: "30", l: `30${t("common.days", lang)}` }, { v: "90", l: `90${t("common.days", lang)}` }].map(o => (
                <button key={o.v} onClick={() => handleSaveSettings({ privacy_auto_delete: parseInt(o.v) })} className={`rounded-xl py-2.5 text-xs font-medium transition-all sm:text-sm ${settings.privacy_auto_delete === parseInt(o.v) ? "border-2 border-copper bg-[var(--color-accent-light)] text-copper" : "border border-[var(--color-border)] bg-[var(--color-bg)] text-[var(--color-text-secondary)]"}`}>{o.l}</button>
              ))}
            </div>
      ...
      <div className="flex h-full flex-col overflow-auto bg-[var(--color-bg)]">
        <div className="w-full px-4 py-6 sm:px-5 sm:py-8 lg:px-6">
          ...
          <div className="mb-6 flex flex-col items-center rounded-2xl bg-[var(--color-surface)] p-6 shadow-sm sm:p-8">
            ...
            <div className="mt-2 rounded-full bg-[var(--color-accent-light)] px-3 py-1 text-xs font-medium text-copper">v1.0.0</div>
          </div>
          <Card className="mb-4">

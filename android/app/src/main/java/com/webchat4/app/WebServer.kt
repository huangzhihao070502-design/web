...
private val MIME = mapOf("html" to "text/html", "js" to "text/javascript", "css" to "text/css",
    ...
    private fun apiCharacterChat(body: String, cors: Map<String, String>): Resp {
        try {
            val j = JSONObject(body); val msg = j.optString("message", "")
            if (msg.isEmpty()) return jsonOk(cors, JSONObject(mapOf("reply" to "请输入消息")))
            val cfg = loadAiConfig()
            if (cfg.optBoolean("enabled") == false || cfg.optString("api_url").isEmpty() || cfg.optString("api_key").isEmpty()) return jsonOk(cors, JSONObject(mapOf("reply" to "请先在 AI 自动回复中配置 API")))

            val pMap = try { JSONObject(personaMapFile.readText()) } catch (_: Exception) { JSONObject() }
            val pId = pMap.optString("character_boss", "")
            var persona: JSONObject? = null
            if (pId.isNotEmpty()) { try { val ps = JSONObject(personaFile.readText()); if (ps.has(pId)) persona = ps.getJSONObject(pId) } catch (_: Exception) {} }
            ...
            val choices = try { resp?.getJSONArray("choices") } catch (_: Exception) { null }
            val reply = try { choices?.getJSONObject(0)?.getJSONObject("message")?.getString("content")?.trim()?.take(500) } catch (_: Exception) { null } ?: "..."
            return jsonOk(cors, JSONObject(mapOf("reply" to reply)))
        } catch (_: Exception) { return jsonOk(cors, JSONObject(mapOf("reply" to "嗯，我现在有点忙，稍后再聊~"))) }
    }
